...
my $result;
 ...
 close(SPACE);	
 print 'x' x 100,"\n";		
 print '~' x 10,"$file è leggibile\n" if -r  $file;
 print '~' x 10,"$file è scrivibile\n" if -w  $file;
 print '~' x 10,"$file ha i permessi di esecuzione\n" if -x  $file;			
 print '~' x 10,"$file è una directory\n" if -d $file;
 print '~' x 10,"$file è di tipo testo\n" if -T $file;
 printf '~' x 10,"$file è stato modificato  %d giorni fà.\n", -M $file;
 print '~' x 10,"$file è  regolare\n" if -f  $file;
